...
const Order = () => {
  ...
  const handleDelete = async (orderId, driverId) => {
    const name = Cookies.get("username");
    const token = Cookies.get(name);
  ...
  const handleFinish = async (orderId, driverId) => {
    const name = Cookies.get("username");
    const token = Cookies.get(name);
    const url = generalUrl + `Order/clientFinished/${orderId}`;
    var obj = {
      isFinish: true,
    };
    await axios
      ...
      .then((response) => {
        ...
        setType("finished");
        setId(driverId);
        Cookies.set("ratingId", driverId, { expires: 30 });
      })
      .catch((error) => {
        console.log(error.response.data.message);
      });
  ...
  async function SendEmailNotification() {
    var message = `${user.userName} ${type} given order.`;
    var url =
                  ...
                  {expandedOrders[index] && (
                    <div className="order-buttons">
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleDelete(order.id, driverId);
                        }}
                      >
                        Delete
                      </button>
                      ...
                      {order.isAccept && (
                        <>
                          {!order.isClientFinish ? (
                            <button
                              className="order-button finish"
                              onClick={(e) => {
                                e.stopPropagation();
                                handleFinish(order.id, driverId);
                              }}
                            >
                              Finish
                            </button>
                          ) : (
                            <span className="finish-wait-message">
                              Wait for to finish
                            </span>
                          )}
                        </>
