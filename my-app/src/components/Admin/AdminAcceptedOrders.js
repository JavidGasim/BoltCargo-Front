...
const AdminAcceptedOrders = () => {
  ...
  const GetAllOrders = async () => {
    ...
    let url = `${generalUrl}Order/acceptedOrders`;
      ...
      try {
        ...
        const ordersWithUserInfo = await Promise.all(
          ordersData.map(async (order) => {
            ...
            let userUrl2 = `${generalUrl}User/${order.driverId}`;

            const userResponse = await axios.get(userUrl, {
              headers: { Authorization: `Bearer ${token}` },
            });

            const userResponse2 = await axios.get(userUrl2, {
              headers: { Authorization: `Bearer ${token}` },
            });
            ...
            return {
              ...
              driver: userResponse2.data.user,
            };
          })
        );
  ...
  return (
    ...
    >
      ...
      <div className="order-card" style={{ marginTop: "35px" }}>
        <h2 className="order-heading">Accepted Orders</h2>
                  ...
                  {expandedOrders[index] && (
                    ...
                    >
                      <div
                        ...
                        style={{ marginRight: "10px" }}
                      >
                        <h1 style={{ fontSize: "0.8em" }}>Driver Info</h1>
                        <img
                          src={orderDriver.imagePath}
                          alt="User"
                          className="user-photo"
                          style={{
                            borderRadius: "50%",
...
export default AdminAcceptedOrders;
